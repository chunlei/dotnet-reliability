...
param([String]$DropPat, [String]$WorkingDirectory, [Switch]$DebuggingBinaries = $false)
...
function Get-TestBinaries([string]$CoreFXBuildMoniker)
{
    ...
    # Get-Drop "dotnet/corefx/master/$LatestCoreFXVersion/tests/anyos/anycpu/netcoreapp1.0" $TestDirectory)
