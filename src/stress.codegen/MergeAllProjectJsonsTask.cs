...
namespace stress.codegen
{
    ...
    public class MergeAllProjectJsonsTask : Task
    {
        ...
        // removes all 'support' types except for the one that is targeted for the run
        // add ALL runtimes we support (this is an AnyOs producer)
        private void ProduceAnyOsProjectJson(string outPath, JObject merged, string targetFramework, string requiredSupport)
        {
            // remove test-runtime if it is present.
            (merged["dependencies"] as JObject)?.Property("test-runtime")?.Remove();

            // remove all supports except the specified one
            merged["supports"]?.Children<JProperty>().Where(x => x.Name != requiredSupport).ToList().ForEach(x => x.Remove());
        ...
        public override bool Execute()
        {
            ...
            ProduceAnyOsProjectJson(OutPath, MergeProjectJsonsIn(InPath), "netstandard1.6", "coreFx.Test.netcoreapp1.0");
